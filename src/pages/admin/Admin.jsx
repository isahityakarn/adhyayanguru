...
export default function AdminPage() {
  ...
  const section = urlSection || searchParams.get("section") || searchParams.get("tab") || "dashboard";
  ...
  const handleBulkDownload = async () => {
    try {
      await secureBulkDownload(
        await bulkDownloadPdfs(selectedPdfs),
  ...
  const handleClassDownload = async (classId) => {
    try {
      await secureBulkDownload(
        await bulkDownloadClass(classId),
        "Class archive downloaded.",
      );
    } catch (requestError) {
...
function UsersView() {
  ...
  const users = [
    ...
    },
    {
      id: 2,
      name: "Rajesh Sharma",
      email: "rajesh.student@gmail.com",
      role: "Student",
      status: "Active",
      initials: "RS",
    },
    ...
    },
    {
      id: 5,
      name: "Sunita Gupta",
      email: "sunita.teacher@gmail.com",
    ...
    <>
      <div className="admin-page-heading">
        <div>
          ...
          <p>
            Manage user accounts, admin access privileges, and student & parent
            profiles.
          </p>
        </div>
          ...
          <tbody>
            {filteredUsers.map((user) => (
              <tr key={user.id}>
                <td>
                  <div className="admin-user">
                    <div
                      ...
                      style={{
                        background:
                          user.role === "Administrator" ? "#e26d4d" : "#42537a",
                      }}
                    >
                      {user.initials}
                    </div>
                ...
                <td>
                  <span
                    className={`admin-pill ${user.role === "Administrator" ? "available" : "pending"}`}
                  >
                    {user.role}
                  </span>
                </td>
                <td>
                  <span className="admin-pill available">{user.status}</span>
                </td>
                <td>
                  <div className="admin-row-actions">
    ...
    <>
      <div className="admin-page-heading">
        <div>
          ...
          <p>
            Configure AI processing models, file storage limits, and server API
            integrations.
          </p>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          >
            <div className="admin-stat-icon" style={{ margin: 0 }}>
              <Sparkles size={18} />
            </div>
            <div>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <span
                style={{ display: "block", fontSize: "11px", color: "#718096" }}
              >
                Upload configuration for chapter materials
              </span>
            </div>
          </div>
          <div
